...
/* map SLICE-space (x:0..114, y:0..150) -> die plane (X:-DWx/2..+, Z:-DWz/2..+) */
var DW = 10.2, DZ = 13.4;
...
(function floorplan(){
  ...
  // blocks extruded from bbox; MM as translucent shelf, others solid on top
  Object.keys(N).forEach(function(id){
    var b=N[id];
    var bw = Math.max(0.5, (b.bbox[2]-b.bbox[0])/114*DW);
    var bd = Math.max(0.5, (b.bbox[3]-b.bbox[1])/150*DZ);
    var cxw = sx((b.bbox[0]+b.bbox[2])/2), czw = sz((b.bbox[1]+b.bbox[3])/2);
    var dens = (b.lut + b.ff*0.4 + b.dsp*90 + b.bram*220) / Math.max(1, (b.bbox[2]-b.bbox[0])*(b.bbox[3]-b.bbox[1]));
    var hgt = b.big ? 0.5 : Math.max(0.28, Math.min(3.4, Math.sqrt(dens)*0.5));
    if(!b.big){ bw=Math.min(bw, 3.0); bd=Math.min(bd, 3.2); }  // clamp overlap sprawl for readability
    ...
    if(bw>1.1 || b.big){
      gFloor.add(textSprite(b.big?"MM_ultra · 24x24":b.name.replace(/^design_1_i\//,"").replace(/_0$/,""),
        b.big?0.62:0.44, "#dfe6f0", cxw, 0.2 + (b.big?0.7:hgt+0.35), czw, true));
    }
  });
})();
...
function buildEdges(){
  EDGES.forEach(function(e){
    ...
    var rad = e[2]==="stream"? 0.03 + e[3]/384*0.05 : 0.016;
    ...
    streamsRT.push({
      ...
      rate: e[2]==="stream" ? 0.11 + e[3]/384*0.20 : 0.24,
      spawn: e[2]==="stream" ? (0.5 + e[3]/70) : 1.1,
...
function inspectPkg(){
  ...
  renderCR(N.mm.cr);
  IEls.note.textContent = "Single-die SoC: dual Cortex-A9 PS + Artix-7 fabric (53 200 LUT, 106 400 FF, 220 DSP48E1, 140 BRAM36). This design fills the DSP column completely.";
  setTim(null);
}
...
function advanceSoc(dt){
  ...
  for(var i=0;i<streamsRT.length && n<PKMAX;i++){
    ...
    var headScale = s.kind==="stream" ? 1.0 + s.bits/384*0.6 : 0.7;
